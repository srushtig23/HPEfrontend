...
class HerSafetyWithBackendSupabaseUser extends BaseAuthUser {
  HerSafetyWithBackendSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> herSafetyWithBackendSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = HerSafetyWithBackendSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
